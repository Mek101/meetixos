...
/**
 * Randomizes the virtual memory layout for the OldKernel's core
 */
pub fn vml_randomize_core_layout(necessary_bitmap_pages: usize) {
    ...
    /* construct the OldKernel text area, which is the only area not randomizable */
    let kern_text_area = {
...
/**
 * Randomizes and returns the VMComponents for the OldKernel core
 */
fn vml_randomize_components(necessary_bitmap_pages: usize)
                            -> [(VMComponent, VMLayoutArea); VMComponent::COUNT] {
    /* calculate the remaining VM space removing the OldKernel text */
    let kern_space_size =
