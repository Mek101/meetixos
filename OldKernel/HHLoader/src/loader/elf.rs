...
/**
 * Loads into this page directory the OldKernel core ELF
 */
pub(super) fn loader_elf_load_core_elf() -> VirtAddr {
    ...
    if let Err(err) = loader_load_elf_segments(preload_cache) {
        panic!("Failed to load OldKernel's core: cause: {}", err);
    }

    /* return the OldKernel entry point */
...
fn loader_load_elf_segments(preload_cache: &KernelPreLoadCache)
                            -> Result<(), &'static str> {
    /* iterate each program header and load the OldKernel */
