...
impl<'a> KernelPreLoadCache<'a> {
    ...
    pub(super) fn new(core_elf_bytes: &'a [u8]) -> Self {
        /* parse the elf bytes and panic when OldKernel core image is corrupted */
        let core_elf = match ElfFile::new(core_elf_bytes) {
            Ok(core_elf) => {
                if let Err(err) = header::sanity_check(&core_elf) {
                    panic!("corrupted OldKernel core image: {}", err);
                }
                core_elf
            },
            Err(err) => panic!("Corrupted OldKernel core image: {}", err)
        };

        /* calculate the necessary memory amount to load the OldKernel core */
        let load_size =
            core_elf.program_iter()
                    .filter_map(|program_hdr| {
                        let hdr_type = match program_hdr.get_type() {
                            Ok(hdr_type) => hdr_type,
                            Err(err) => panic!("Malformed OldKernel core header: {}", err)
                        };
    ...
    /**
     * Returns the memory load size for the OldKernel
     */
