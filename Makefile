...
run_dbg: image
	...
	$(V) $(QEMU) $(VIRT_ACCEL) $(QEMU_ARGS) -S -s -cdrom $(BUILD_PREFIX)/$(BUILD_MODE)/meetixos.iso &
	$(V) $(R_DBG) $(BUILD_PREFIX)/kernel/$(BUILD_MODE)/mx_kernel --command=$(BUILD_PREFIX)/$(BUILD_MODE)/gdb_commands.txt
