...
pub enum RecvMode {
    /**
     * Simply asks to the Kernel whether an object of the requested type is
     * already available into the object receiving queue, if not it returns
     * an `Err(NoDataAvailable)` error
     */
    Poll,

    /**
     * Puts the task in a waiting state until an object of the
