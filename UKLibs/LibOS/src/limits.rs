 ...
 * at the same time for the same object
 */
...
/**
 * Maximum length in bytes for an `LibApi::ents::OSEntity` name
 */
pub const ENTITY_NAME_LEN_MAX: usize = 64;

/**
 * Maximum amount of `LibApi::ents::impls::OSGroup`s for each
 * `LibApi::ents::impls::OSUser`
 */
